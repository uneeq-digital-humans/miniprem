...
class TelemetryService:
    ...
    async def _get_renny_scene_info(self) -> Optional[str]:
        """
        Query Renny's health endpoint to get current scene/avatar name.

        Tries common Renny API endpoints to get scene information.

        Returns:
            Scene name string or None if not available
        """
        # Common Renny API endpoints to try
        renny_endpoints = [
            "http://renny:8000/health",
            "http://renny:8000/api/health",
            ...
            try:
                async with httpx.AsyncClient(timeout=2.0) as client:
                    ...
                    if response.status_code == 200:
                        data = response.json()
                        # Try different possible field names for scene
                        scene = (
                            data.get('scene') or
                            data.get('scene_name') or
                            data.get('avatar') or
                            data.get('avatar_name') or
                            data.get('character') or
                            data.get('persona')
                        )
                        if scene:
