...
# Get machine ID from GPU UUID (or hostname fallback)
get_machine_id() {
    local gpu_uuid=""
    ...
    if [ -n "$gpu_uuid" ]; then
        ...
        log "WARN: Machine ID from hostname (GPU not available)"
    fi
}
...
if send_event "installation" "\"version\": \"renny-0.713\", \"source\": \"docker-compose\""; then
