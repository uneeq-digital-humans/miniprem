...
pull_required_images() {
    ...
    if [ "$INSTALL_TYPE" = "full" ]; then
        ...
        if ! pull_images_parallel "cr.uneeq.io/uneeq/renny-renderer:0.1332-decd6"; then
...
# Generate docker/flowise.env with random secrets and admin credentials.
# Flowise 3.x uses a first-run admin account (email + password) instead of the
# legacy FLOWISE_USERNAME/FLOWISE_PASSWORD basic auth. The JWT/session secrets
# must be stable across container restarts or every restart invalidates logins.
ensure_flowise_env_exists() {
    local env_file="$PROJECT_ROOT/docker/flowise.env"

    if stat "$env_file" > /dev/null 2>&1; then
        info "Flowise environment file already exists at $env_file"
        return 0
    fi

    info "Generating Flowise secrets and admin credentials..."

    # Password must satisfy Flowise complexity rules: lower, upper, digit, special
    local admin_password="Mp1!$(openssl rand -hex 8)"

    cat > "$env_file" <<EOF
# Generated by install_miniprem.sh on $(date '+%Y-%m-%d %H:%M:%S')
# See flowise.env.example for documentation. Do not commit this file.

# Encryption key for credentials stored in Flowise (changing it makes stored
# credentials undecryptable — wipe the flowise_data volume if you do)
FLOWISE_SECRETKEY_OVERWRITE=$(openssl rand -hex 32)

# Auth/session secrets — must stay stable across container restarts
...
# Initial Flowise admin account (created automatically during chatflow setup).
# Use these to log in at http://localhost:3000
FLOWISE_ADMIN_EMAIL=admin@miniprem.local
FLOWISE_ADMIN_PASSWORD=${admin_password}
EOF
    chmod 600 "$env_file"

    if [ ! -s "$env_file" ]; then
        fatal "Failed to create $env_file. Check disk space and permissions."
    fi
    success "$CHECKMARK Created $env_file with generated secrets"
    info "Flowise admin login is stored in $env_file (FLOWISE_ADMIN_EMAIL / FLOWISE_ADMIN_PASSWORD)"
}
...
main() {
    ...
    # Full installs run Flowise, which needs generated secrets and admin credentials
    if [ "$INSTALL_TYPE" = "full" ]; then
        ensure_flowise_env_exists
    fi
    ...
    if [ "$INSTALL_TYPE" = "full" ]; then
        ...
        info "  Log in with the admin account in docker/flowise.env (FLOWISE_ADMIN_EMAIL / FLOWISE_ADMIN_PASSWORD)."
        info "  If no account exists yet, the first visit prompts you to create one."
