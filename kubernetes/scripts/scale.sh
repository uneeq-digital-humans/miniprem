...
#
# Usage:
#   ./scale.sh <desired_count>
#   ./scale.sh [OPTIONS] <desired_count>
#
# Examples:
#   ./scale.sh 15                    # Scale to 15 nodes
#   ./scale.sh --component renny 12  # Scale Renny to 12 nodes
#
...
select_platform() {
    print_color "$BOLD" "
╔═══════════════════════════════════════════════════════════════╗
║          MiniPrem Multi-Cloud Kubernetes Scaling              ║
╚═══════════════════════════════════════════════════════════════╝
"
...
delegate_to_platform_script() {
    local script_path="$SCRIPT_DIR/$SCALING_SCRIPT"

    info "Delegating to platform-specific script: $SCALING_SCRIPT"

    # Check if platform-specific script exists
    if [[ ! -f "$script_path" ]]; then
        error "Platform-specific scaling script not found: $script_path"
        echo ""

        if [[ "$PLATFORM" == "gcp" ]]; then
            warning "GCP scaling support is planned but not yet implemented."
            echo ""
            echo "To track progress or contribute:"
            echo "  - Check for open issues in the repository"
            echo "  - See documentation for AWS/Azure scaling as reference"
            exit 1
        else
            error "Expected script location: $script_path"
            echo "Please ensure the scaling scripts are properly installed."
            exit 1
        fi
    fi
    ...
    print_color "$BOLD" "
╔═══════════════════════════════════════════════════════════════╗
║              Starting $PLATFORM_NAME Scaling                   ║
╚═══════════════════════════════════════════════════════════════╝
"
...
main() {
    ...
    # Delegate to platform-specific scaling script with all arguments
    delegate_to_platform_script "$@"
}
